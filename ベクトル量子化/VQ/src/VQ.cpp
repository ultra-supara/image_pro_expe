...
#define BLOCK_SIZE 8      /* 想定ブロックサイズ 4 x 4 画素 */
#define DIMENSION 64     /* ベクトルの次元（＝ 4 x 4 ）   */
...
/* [y][x]ブロックのベクトルとNo.n 代表ベクトル間の距離計算 */
{
    ...
    for (i = 0; i < DIMENSION; i++)
        sum = sum + (vector[y][x][i] - main_vector[n][i]) * (vector[y][x][i] - main_vector[n][i]);
    return (sqrt(sum));
}
...
/* K-平均法によるクラスタリングを行う */
{
    ...
    while (!finish) {
        ...
        for (i = 0; i < K_number; i++)
        {
            ...
            for (j = 0; j < DIMENSION; j++)
            {
                difference = difference + abs(init_main_vector[i][j] - main_vector[i][j]);
            }
            ...
            for (y = 0; y < y_block; y++)
            {
                for (x = 0; x < x_block; x++)
                {
                    ...
                    for (i = 0; i < BLOCK_SIZE; i++)
                    {
                        for (j = 0; j < BLOCK_SIZE; j++)
                        {
                            image2[ys + i][xs + j] = main_vector[cluster[y][x]][i * BLOCK_SIZE + j];
                        }
                    }
                }
            }
    ...
    printf("元の階調画像のデータ量=%d\n", original_data_size);
    i = K_number - 1;    j = 0;
    do{
        i = i / 2;
        j++;
    } while ( i > 0 );  /* j : 代表ベクトルの指定に必要なビット数 */
    coded_data_size = BLOCK_SIZE * BLOCK_SIZE * j + j * x_block * y_block;
    /* 代表ベクトル(符号化データ) */
	/* ルックアップテーブル (代表ベクトルの指定)*/
    printf("圧縮データ（画像部分）のデータ量 = %d\n", coded_data_size);
    compression_rate = (double)coded_data_size / (double)original_data_size * 100.0;
    printf("圧縮率：%f [％]\n",compression_rate);
}
void signal_to_noise_ratio(){
    ...
    for (i = 0; i < x_size1; i++){
        for (j = 0; j < y_size1; j++)  {
            sum1 += (image1[i][j]) * (image1[i][j]);
            sum2 += abs(image2[i][j] - image1[i][j]) * abs(image2[i][j] - image1[i][j]);
        }
    }
    SNR = 10 * (log10(sum1) - log10(sum2));
    printf("SNR : %f [dB] \n",SNR);
}
...
int main() {
    ...
    signal_to_noise_ratio(); /* SNRの表示をする関数 */
